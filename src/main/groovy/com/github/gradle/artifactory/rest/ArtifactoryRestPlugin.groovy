...
import com.github.gradle.artifactory.rest.utils.ArtifactoryRestThreadContextClassLoader
import com.github.gradle.artifactory.rest.utils.ThreadContextClassLoader
...
class ArtifactoryRestPlugin implements Plugin<Project> {
    ...
    private static Set<File> configurePluginClassPath(Project project) {
        project.configurations.getByName(ARTIFACTORY_CONFIGURATION_NAME).defaultDependencies { dependencies ->
            dependencies.add(project.dependencies.create("com.github:artifactory-rest:$ArtifactoryRestPlugin.ARTIFACTORY_JAVA_DEFAULT_VERSION"))
            dependencies.add(project.dependencies.create('org.slf4j:slf4j-simple:1.7.5'))
            dependencies.add(project.dependencies.create('commons-io:commons-io:2.4'))
        }.files
    }
}
